...
from src.mapping.tranformer import Transformer
...
class V12001(Transformer):
    ...
    def _anti_tel_apply_confin_7d(self, df=None):
        df1 = df.loc[df['rule_name'] == '手机号七天内多头申请过多', 'rule_memo'].copy()
        if len(df1) > 0:
            self.variables['anti_tel_apply_confin_7d'] = get_confin1_count(df1.values[0]) + get_confin2_count(df1.values[0])

    # 计算反欺诈_身份证7天内申请消费金融次数   ***
    def _anti_id_apply_confin_7d(self, df=None):
        df2 = df.loc[df['rule_name'] == '身份证七天内多头申请过多', 'rule_memo'].copy()
        if len(df2) > 0:
            self.variables['anti_id_apply_confin_7d'] = get_confin1_count(df2.values[0]) + get_confin2_count(df2.values[0])

    # 计算反欺诈_手机号1个月内申请消费金融次数  ***
    def _anti_tel_apply_confin_1m(self, df=None):
        df3 = df.loc[df['rule_name'] == '手机号一个月内多头申请过多', 'rule_memo'].copy()
        if len(df3) > 0:
            self.variables['anti_tel_apply_confin_1m'] = get_confin1_count(df3.values[0]) + get_confin2_count(df3.values[0])

    # 计算反欺诈_身份证1个月内申请消费金融次数   ***
    def _anti_id_apply_confin_1m(self, df=None):
        df4 = df.loc[df['rule_name'] == '身份证一个月内多头申请过多', 'rule_memo'].copy()
        if len(df4) > 0:
            self.variables['anti_id_apply_confin_1m'] = get_confin1_count(df4.values[0]) + get_confin2_count(df4.values[0])

    # 计算反欺诈_手机号3个月内申请消费金融次数  ***
    def _anti_tel_apply_confin_3m(self, df=None):
        df5 = df.loc[df['rule_name'] == '手机号三个月内多头申请过多', 'rule_memo'].copy()
        if len(df5) > 0:
            self.variables['anti_tel_apply_confin_3m'] = get_confin1_count(df5.values[0]) + get_confin2_count(df5.values[0])

    # 计算反欺诈_身份证3个月内申请消费金融次数   ***
    def _anti_id_apply_confin_3m(self, df=None):
        df6 = df.loc[df['rule_name'] == '身份证三个月内多头申请过多', 'rule_memo'].copy()
        if len(df6) > 0:
            self.variables['anti_id_apply_confin_3m'] = get_confin1_count(df6.values[0]) + get_confin2_count(df6.values[0])
    ...
    def _anti_apply_bank_7d(self):
        self.variables['anti_apply_bank_7d'] = max(self.variables['anti_tel_apply_bank_7d'], self.variables['anti_id_apply_bank_7d'])

    # 计算反欺诈_1个月内申请银行次数   ***
    def _anti_apply_bank_1m(self):
        self.variables['anti_apply_bank_1m'] = max(self.variables['anti_tel_apply_bank_1m'], self.variables['anti_id_apply_bank_1m'])

    # 计算反欺诈_3个月内申请银行次数   ***
    def _anti_apply_bank_3m(self):
        self.variables['anti_apply_bank_3m'] = max(self.variables['anti_tel_apply_bank_3m'], self.variables['anti_id_apply_bank_3m'])

    # 计算反欺诈_7天内申请小贷次数   ***
    def _anti_apply_sloan_7d(self):
        self.variables['anti_apply_sloan_7d'] = max(self.variables['anti_tel_apply_sloan_7d'], self.variables['anti_id_apply_sloan_7d'])

    # 计算反欺诈_1个月内申请小贷次数   ***
    def _anti_apply_sloan_1m(self):
        self.variables['anti_apply_sloan_1m'] = max(self.variables['anti_tel_apply_sloan_1m'], self.variables['anti_id_apply_sloan_1m'])

    # 计算反欺诈_3个月内申请小贷次数   ***
    def _anti_apply_sloan_3m(self):
        self.variables['anti_apply_sloan_3m'] = max(self.variables['anti_tel_apply_sloan_3m'], self.variables['anti_id_apply_sloan_3m'])

    # 计算反欺诈_7天内申请p2p次数   ***
    def _anti_apply_p2p_7d(self):
        self.variables['anti_apply_p2p_7d'] = max(self.variables['anti_tel_apply_p2p_7d'], self.variables['anti_id_apply_p2p_7d'])

    # 计算反欺诈_1个月内申请p2p次数   ***
    def _anti_apply_p2p_1m(self):
        self.variables['anti_apply_p2p_1m'] = max(self.variables['anti_tel_apply_p2p_1m'], self.variables['anti_id_apply_p2p_1m'])

    # 计算反欺诈_3个月内申请p2p次数   ***
    def _anti_apply_p2p_3m(self):
        self.variables['anti_apply_p2p_3m'] = max(self.variables['anti_tel_apply_p2p_3m'], self.variables['anti_id_apply_p2p_3m'])

    # 计算反欺诈_7天内申请消费金融次数   ***
    def _anti_apply_confin_7d(self):
        self.variables['anti_apply_confin_7d'] = max(self.variables['anti_tel_apply_confin_7d'], self.variables['anti_id_apply_confin_7d'])

    # 计算反欺诈_1个月内申请消费金融次数   ***
    def _anti_apply_confin_1m(self):
        self.variables['anti_apply_confin_1m'] = max(self.variables['anti_tel_apply_confin_1m'], self.variables['anti_id_apply_confin_1m'])

    # 计算反欺诈_3个月内申请消费金融次数   ***
    def _anti_apply_confin_3m(self):
        self.variables['anti_apply_confin_3m'] = max(self.variables['anti_tel_apply_confin_3m'], self.variables['anti_id_apply_confin_3m'])

    # 计算反欺诈_7天内申请其他机构次数   ***
    def _anti_apply_other_7d(self):
        self.variables['anti_apply_other_7d'] = max(self.variables['anti_tel_apply_other_7d'], self.variables['anti_id_apply_other_7d'])

    # 计算反欺诈_1个月内申请其他机构次数   ***
    def _anti_apply_other_1m(self):
        self.variables['anti_apply_other_1m'] = max(self.variables['anti_tel_apply_other_1m'], self.variables['anti_id_apply_other_1m'])

    # 计算反欺诈_3个月内申请其他机构次数   ***
    def _anti_apply_other_3m(self):
        self.variables['anti_apply_other_3m'] = max(self.variables['anti_tel_apply_other_3m'], self.variables['anti_id_apply_other_3m'])
