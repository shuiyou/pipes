...
class T12001(Transformer):
    ...
    def _info_anti_fraud_rule(self):
        sql = '''
            SELECT info_anti_fraud_rule.rule_name,info_anti_fraud_rule.rule_memo FROM info_anti_fraud_rule LEFT JOIN   
            info_anti_fraud_strategy ON info_anti_fraud_rule.anti_fraud_rule_id = info_anti_fraud_strategy.anti_fraud_rule_id
            WHERE info_anti_fraud_strategy.anti_fraud_id IN (SELECT af.anti_fraud_id FROM (SELECT anti_fraud_id FROM info_anti_fraud 
    ...
    def _anti_idno_apply_1m(self, df=None):
        ...
        self.variables['anti_idno_apply_1m'] = df.loc[df['rule_name'] == '身份证一个月内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_身份证三个月内多头申请过多_次数
    def _anti_idno_apply_3m(self, df=None):

        self.variables['anti_idno_apply_3m'] = df.loc[df['rule_name'] == '身份证三个月内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_手机号一个月内多头申请过多_次数
    def _anti_tel_apply_1m(self, df=None):

        self.variables['anti_tel_apply_1m'] = df.loc[df['rule_name'] == '手机号一个月内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_手机号三个月内多头申请过多_次数
    def _anti_tel_apply_3m(self, df=None):

        self.variables['anti_tel_apply_3m'] = df.loc[df['rule_name'] == '手机号三个月内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_身份证三天内多头申请过多_次数
    def _anti_idno_apply_3d(self, df=None):

        self.variables['anti_idno_apply_3d'] = df.loc[df['rule_name'] == '身份证三天内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_身份证七天内多头申请过多_次数
    def _anti_idno_apply_7d(self, df=None):

        self.variables['anti_idno_apply_7d'] = df.loc[df['rule_name'] == '身份证七天内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_手机号三天内多头申请过多_次数
    def _anti_tel_apply_3d(self, df=None):

        self.variables['anti_tel_apply_3d'] = df.loc[df['rule_name'] == '手机号三天内多头申请过多','rule_memo'].split(',')[0].split('：')[1]

    # 计算反欺诈_手机号七天内多头申请过多_次数
    def _anti_tel_apply_7d(self, df=None):

        self.variables['anti_tel_apply_7d'] = df.loc[df['rule_name'] == '手机号七天内多头申请过多','rule_memo'].split(',')[0].split('：')[1]
