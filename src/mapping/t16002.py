...
class T16002(Transformer):
    ...
    #行政违法记录sql
    def _court_administrative_violation_df(self,unique_name):
        ...
        FROM info_court_administrative_violation B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
    ...
    def _court_judicative_pape_df(self,unique_name):
        ...
        FROM info_court_judicative_pape B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    def _court_trial_process_df(self,unique_name):
        ...
        FROM info_court_trial_process B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    def _court_taxable_abnormal_user_df(self,unique_name):
        ...
        confirm_date
        FROM info_court_taxable_abnormal_user B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    #欠款欠费名单sql
    def _court_arrearage_df(self,unique_name):
        info_court_arrearage = """
        SELECT A.create_time as create_time,B.default_amount as
        default_amount,B.default_date as default_date
        FROM info_court_arrearage B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    #欠税名单sql
    def _court_tax_arrears_df(self,unique_name):
        info_court_tax_arrears = """
        SELECT A.create_time as create_time,B.taxes as
        taxes,B.taxes_time as taxes_time
        FROM info_court_tax_arrears B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    def _court_deadbeat_df(self,unique_name):
        ...
        FROM info_court_deadbeat B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    #限制出入境sql
    def _court_limited_entry_exit_df(self,unique_name):
        info_court_limited_entry_exit = """
        SELECT A.create_time as create_time,B.execute_content as
        execute_content,B.specific_date as specific_date
        FROM info_court_limited_entry_exit B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    #限制高消费sql
    def _court_limit_hignspending_df(self,unique_name):
        info_court_limit_hignspending = """
        SELECT A.create_time as create_time,B.execute_content as
        execute_content,B.specific_date as specific_date
        FROM info_court_limit_hignspending B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    #执行公开信息sql
    def _court_excute_public_df(self,unique_name):
        info_court_excute_public = """
        SELECT A.create_time as create_time,B.execute_content as
        execute_content,B.filing_time as filing_time
        FROM info_court_excute_public B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    def _court_criminal_suspect_df(self,unique_name):
        ...
        trial_date
        FROM info_court_criminal_suspect B,(SELECT create_time FROM info_court WHERE unique_name = %(unique_name)s
        AND expired_at > NOW() ORDER BY expired_at DESC LIMIT 1) A
        WHERE B.court_id = A.id
        """
    ...
    def transform(self):
        ...
        self._ps_court_taxable_abnormal_user(self._court_tax_arrears_df(self.unique_name))
        self._ps_court_arrearage(self._court_arrearage_df(self.unique_name))
        self._ps_court_deadbeat(self._court_deadbeat_df(self.unique_name))
        self._ps_court_limited_entry_exit(self._court_limited_entry_exit_df(self.unique_name))
        self._ps_court_limit_hignspending(self._court_limit_hignspending_df(self.unique_name))
        self._ps_court_excute_public(self._court_excute_public_df(self.unique_name))
        self._ps_court_criminal_suspect(self._court_criminal_suspect_df(self.unique_name))
